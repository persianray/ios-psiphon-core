...
TEMP_DIR=$(mktemp -d)
mkdir -p $TEMP_DIR/src/github.com/Psiphon-Labs/psiphon-tunnel-core/psiphon/common/quic/gquic-go/

# uppercase the name of the interface
INTERFACE_NAME="$(tr '[:lower:]' '[:upper:]' <<< ${4:0:1})${4:1}"

# copy all .go files to a temporary directory
rsync -r --exclude 'vendor' --include='*.go' --include '*/' --exclude '*'   $GOPATH/src/github.com/Psiphon-Labs/psiphon-tunnel-core/psiphon/common/quic/gquic-go/ $TEMP_DIR/src/github.com/Psiphon-Labs/psiphon-tunnel-core/psiphon/common/quic/gquic-go/
...
echo "type $INTERFACE_NAME = $4" >> $TEMP_DIR/src/github.com/Psiphon-Labs/psiphon-tunnel-core/psiphon/common/quic/gquic-go/mockgen_interface.go
